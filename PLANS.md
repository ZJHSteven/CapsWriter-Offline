...
### 目标
- 解决“云端正常 `task-finished`，但最终识别文本为空”无法追责的问题。
- 用本地音频 RMS/峰值区分“疑似没录到声音”和“检测到声音但云端空识别”。
- 对有声音的空识别自动重试一次；重试仍空时保留失败任务目录供复盘。

### 已完成
- [x] 实时会话累计音频诊断字段：样本数、RMS、峰值、有声帧比例。
- [x] 空文本成功改判：
  - [x] 近静音：`failed_silent_audio`，不自动重试，不落无意义失败目录。
  - [x] 有声音：`failed_empty_result`，允许自动重试；重试仍失败时落盘。
- [x] 失败任务 `meta.json` 和 `ASR_WS` 摘要写入音频诊断。
- [x] 客户端终端按状态打印清晰诊断信息。
- [x] 单元测试覆盖静音空结果、有声空结果、正常成功路径和既有 finish 等待策略。

### 后续观察
- [ ] 用真实麦克风故意静音录一段，确认终端显示“疑似未录到有效声音”，且不生成失败任务目录。
- [ ] 用真实说话继续观察一段时间，若再出现空识别，确认自动重试和失败落盘是否按预期触发。
- [ ] 根据真实日志里的 RMS/peak 分布，必要时微调 `voice_rms_threshold` / `voice_peak_threshold`。
